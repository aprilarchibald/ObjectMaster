...
const name = pokemon.map(p => p.name)
// console.log(name)
...
const poisonType = pokemon.filter(p => p.types.length ==1 && p.types.includes("poison")).map(p => p.name);
// console.log(poisonType)

// an array containing just the first type of all the pokémon whose second type is "flying"
const array = pokemon.filter(p => p.types[1] === "flying").map(p => p.types[0]);
// console.log(array)
